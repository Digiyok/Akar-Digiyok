        <nav class="navbar navbar-expand navbar-light bg-navbar topbar mb-4 static-top">
          ...
          <ul class="navbar-nav ml-auto">
              ...
              <div class="dropdown-menu dropdown-menu-right p-3 shadow animated--grow-in" aria-labelledby="searchDropdown">
                <form class="navbar-search">
                  <div class="input-group">
                    <input class="form-control bg-light border-1 small" type="text" aria-label="Search" aria-describedby="basic-addon2" placeholder="Apa yang Anda cari?">
            ...
            <li class="nav-item dropdown no-arrow mx-1">
              <a class="nav-link dropdown-toggle" id="messagesDropdown" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">
              ...
              <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="messagesDropdown">
                ...
                <a class="dropdown-item d-flex align-items-center" href="#">
                  <div class="dropdown-list-image mr-3">
                    <img class="rounded-circle" src="{{ asset('img/man.png') }}" alt="" style="max-width: 60px">
                ...
                <a class="dropdown-item d-flex align-items-center" href="#">
                  <div class="dropdown-list-image mr-3">
                    <img class="rounded-circle" src="{{ asset('img/boy.png') }}" alt="" style="max-width: 60px">
            ...
            @if (Auth::user()->role->name != 'keulsi')
              <li class="nav-item no-arrow mx-1">
                <a class="nav-link" href="https://bit.ly/36iredy" target="_blank">
            ...
            @if (Auth::user()->role->name == 'keulsi')
              <li class="nav-item dropdown no-arrow mx-1">
                ...
                <div class="dropdown-list dropdown-menu dropdown-menu-right shadow change-role animated--grow-in"
                  aria-labelledby="changeSystemDropdown">
                  <h6 class="dropdown-header">
                    Beralih Unit
                  </h6>
                  @php
                    $color = ['warning', 'brand-green', 'brand-purple', 'info', 'brand-green-dark', 'danger'];
                    $i = 0;
                  @endphp
                  @foreach (Auth::user()->pegawai->units()->orderBy('unit_id')->get() as $u)
                    @if (Auth::user()->pegawai->unit_id == $u->unit->id)
                      <a class="dropdown-item d-flex align-items-center disabled" href="#">
                        ...
                        <div>
                          <span class="font-weight-normal">{{ $u->unit->name }}</span><span
                            class="text-success ml-2">•</span>
                        </div>
                      </a>
                    @else
                      <a class="dropdown-item d-flex align-items-center"
                        href="{{ route('akun.unit.change', ['unit' => $u->unit->name]) }}">
                        <i class="fas fa-school text-{{ $color[$i] }} mr-2"></i>
                        <div>
                      ...
                      @foreach ($u->jabatans()->orderBy('position_id')->get() as $j)
                        @if (Auth::user()->pegawai->unit_id == $u->unit->id && Auth::user()->pegawai->position_id == $j->id)
                          <a class="dropdown-item d-flex align-items-center disabled" href="#">
                            ...
                            <div>
                              <span class="font-weight-normal">{{ $j->name }}</span><span
                                class="text-success ml-2">•</span>
                            </div>
                          </a>
                        @else
                          <a class="dropdown-item d-flex align-items-center"
                            href="{{ route('akun.role.change', ['unit' => $u->unit->name, 'position' => Crypt::encryptString($j->id)]) }}">
                            <i class="fas fa-user-check text-{{ $color[$i] }} mr-2"></i>
                            <div>
            ...
            @if (Auth::user()->role->name != 'keulsi')
              <li class="nav-item dropdown no-arrow mx-1">
                ...
                <div class="dropdown-list dropdown-menu dropdown-menu-right shadow change-system animated--grow-in"
                  aria-labelledby="changeSystemDropdown">
                  <h6 class="dropdown-header">
                    Pindah Sistem
                  </h6>
                  ...
                  {{-- <a class="dropdown-item d-flex align-items-center" href="{{ url('/kependidikan') }}">
                    <div class="mr-3">
                      <i class="zmdi zmdi-graduation-cap zmdi-hc-2x text-warning"></i>
                    </div>
                    <div>
                      <span class="font-weight-bold">Kependidikan</span>
                    </div>
                  </a> --}}
                  <a class="dropdown-item d-flex align-items-center" href="{{ url('/keuangan') }}">
                    <div class="mr-3">
                      <i class="zmdi zmdi-balance-wallet zmdi-hc-2x text-brand-green-dark"></i>
                    </div>
                    <div>
            ...
            <li class="nav-item dropdown no-arrow">
              <a class="nav-link dropdown-toggle" id="userDropdown" data-toggle="dropdown" href="#"
                ...
                <div class="img-profile" style="max-width: 60px">
                  <img class="avatar-img rounded-circle" src="{{ asset(Auth::user()->pegawai->showPhoto) }}">
                </div>
                <span class="ml-2 d-none d-lg-inline text-white small">{{ Auth::user()->pegawai->nickname }}</span>
              </a>
              <div class="dropdown-menu dropdown-menu-right shadow animated--grow-in dropdown-profile"
                ...
                <div class="px-4 py-2 d-flex align-items-center">
                  ...
                  <div>
                    ...
                    <h6 class="mb-0"><i
                        class="mdi mdi-office-building mr-2 text-brand-blue-dark"></i>{{ Auth::user()->pegawai->unit->name }}
                    </h6>
                  </div>
                </div>
