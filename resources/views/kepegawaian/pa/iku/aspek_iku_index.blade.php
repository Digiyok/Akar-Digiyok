              ...
              <div class="form-group mb-0">
                <div class="row">
                  <div class="col-lg-3 col-md-4 col-12">
                    <label class="form-control-label" for="categoryOpt">IKU</label>
                  </div>
                  <div class="col-lg-9 col-md-8 col-12">
                    <div class="input-group">
                      <select class="form-control" id="categoryOpt" name="iku" aria-label="IKU" required="required">
                        @foreach ($categories as $c)
                          <option value="{{ $c->nameLc }}" {{ $c->name == $iku->name ? 'selected' : '' }}>
                            {{ $c->name }}</option>
                        @endforeach
                      </select>
                      <a class="btn btn-brand-green ml-2 pt-2" id="btn-select-category"
                        data-href="{{ route('iku.aspek.index') }}" href="{{ route('iku.aspek.index') }}">Pilih</a>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div class="row py-2 rounded border border-light mr-2">
                  <div class="col-8 d-flex align-items-center">
                    ...
                    <div>
                      <a class="font-weight-bold text-dark"
                        href="{{ route('iku.aspek.index', ['iku' => $iku->nameLc, 'unit' => $u->name]) }}">
                        {{ $u->name }}
                      </a>
                    </div>
                  </div>
                  <div class="col-4 d-flex justify-content-end align-items-center">
                    <a class="btn btn-sm btn-outline-brand-green-dark"
                      href="{{ route('iku.aspek.index', ['iku' => $iku->nameLc, 'unit' => $u->name]) }}">Pilih</a>
                  </div>
                </div>
              </div>
            @endforeach
